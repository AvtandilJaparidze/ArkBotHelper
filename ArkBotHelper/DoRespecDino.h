...
std::string DoRespecDinoInternal(FString* cmd, APlayerController* aPlayerController)
{
	...
	else if (aPlayerController)
	{
		...
		auto character = aShooterPlayerController->GetPlayerCharacter();
	...
	if (!dino)
	{
		...
		if (!dino) return "Could not find a dino with supplied id";
	}

	UPrimalCharacterStatusComponent* status = dino->MyCharacterStatusComponentField();
	auto statusEx = reinterpret_cast<UPrimalCharacterStatusComponent*>(status);
	if (status)
	{
		// 0: health
		// 1: stamina
		...
		auto func_ClientNotifyLevelUpPC = aShooterPlayerController->GetPlayerCharacter()->FindFunctionChecked(FName(L"ClientNotifyLevelUp", EFindName::FNAME_Find, true));
		if (func_ClientNotifyLevelUpPC) aShooterPlayerController->GetPlayerCharacter()->ProcessEvent(func_ClientNotifyLevelUpPC, &TArray<unsigned short>());
		...
		return "Successfully triggered dino respec";
	}
